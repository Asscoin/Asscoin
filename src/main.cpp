...
#if defined(NDEBUG)
# error "Asscoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x40104904a16d007f430e5112dc3e87d8948556833ad267a830b70ee66667ac01");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Asscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Asscoin Signed Message:\n";
...
{
    // Asscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 45 * COIN;

            if(nHeight == 1)
            {
            nSubsidy = 28526087 * COIN;
            }

    // Subsidy is cut in half every 270000 blocks
    nSubsidy >>= (nHeight / 270000);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Asscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Asscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1471419907, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "There is nothing better in the world than a beautiful ass";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 45 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("045e60a6ab01631fb24ee66f327af5ce3305459fc0cc6032b41524897de40d7bfe700c194c7fa999635f21ca19235fbb743d2001d9b5ecc04aae1220b5c989a2f0") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1471419907;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1733458;
        ...
        assert(block.hashMerkleRoot == uint256("0xbed55ab6639daeb18a7c059624b1f82950f4a6c1a5e75556df05356dcfdb1dd6"));
...
unsigned char pchMessageStart[4] = { 0x7d, 0x8f, 0xa3, 0xbc }; // Asscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AsscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AsscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AsscoinMiner(CWallet *pwallet)
{
    printf("AsscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("asscoin-miner");
    ...
    try { loop {
        ...
        printf("Running AsscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AsscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AsscoinMiner, pwallet));
}
